...
class testPlugin(TrustHubPlugin):
    ...
    def query(self, host, port, cert_chain):
        ...
        self.write_to_file(cert_chain,'C:/tmp/certchain')
    ...
    def validate_certs(self,uri):
        cert_file = "C:/tmp/cert.pem"
        chain_file = "C:/tmp/chain.pem"
        sign_file = "C:/tmp/signcert.pem"
        server = uri.split("/")[2]
        output = ""
        with open("C:/tmp/error.txt", 'w') as f:
    ...
    def check_error(self):
        error = False
        with open("C:/tmp/error.txt",'r') as f:
    ...
    def verify_output(self,text):
        for line in text.split("\n"):
            if line.startswith("C:/tmp/cert.pem:"):
                fields = line.split(" ")
                if fields[1] == "good":
                    print "OK"
    ...
    def write_to_files(self,certs):
        ...
        self.write_to_file(root,'C:/tmp/cert.pem')
        certp = ''
        for cert in certs[1:]:
            certp += crypto.dump_certificate(crypto.FILETYPE_PEM,cert)
        self.write_to_file(certp,'C:/tmp/chain.pem')
