...
int main(int argc, char* argv[]) {
	...
	if (choice == 'n') {
		printf("\r\nResponding to requests as invalid\r\n");
	} else {
		printf("\r\nResponding to requests as valid\r\n");
	}
	
	printf("Save the first query to a file?\r\n>>(y/n)");
	char save_one;
	while ((save_one = getchar()) == '\n');
	if (save_one == 'n') {
		printf("\r\nNot saving\r\n");
		save_one = false;
	} else {
		printf("\r\nWill save first query to file\r\n");
		save_one = true;
	}
